...
#prediction and response functions
...
def happy_response():
  ...
  return "\n{intro}\n{r1}\n{gif}".format(intro = intro, r1 = r1,gif =gif)
...
def get_bot_response():
    ...
    for word in hello_words:
        if userText.startswith(word): 
            return 'Hello! How are you, lovely person \N{smiling face with halo}'
    
    if userText.startswith("$inspire"): 
        quote = get_quote()
        return quote

    elif userText.startswith('$joy'): 
            quote = get_quote()
    ...
    elif any(word in userText for word in lazy_words): #iterates over lazy_words to check if there is any word in message that is also in lazy_words
        response = random.choice(exercises)
        return (response) #gives a random excercise suggestion
    ...
    elif ("meme" in userText) or ("memes" in userText):
    ...
    elif any(word in userText for word in relationship_words):
        return ("Relationships are complicated and are very precious. We might not be able to understand but ask for advice in the safespace discord server of therapists and kind folks: https://discord.gg/9g98gZ9H%22")
    ...
    elif  ("no" in userText) and ("not" not in userText) and ("now" not in userText):
        return ("Try typing $joy for some other cool stuff ") 

    elif contains_depression_traces(userText):
            response = depression_response()
            return (response)
    elif contains_stress_traces(userText):
            response = stress_response()
            return (response)
    elif contains_worry_traces(userText):
            response = worry_response()
            return (response)
    elif contains_happy_traces(userText):
        response = happy_response()
        return (response)
    elif ("thank you" in userText) or ("thank u" in userText) or ("thank you!" in userText) or ("thanks" in userText):
        return ("My Pleasure \N{smiling face with halo}")
    else:
